window.requestAnimFrame = (function() {
  ...
  var h = c.height = window.innerHeight*(2.5);
  var _w = w * 0.5;
  var _h = h * 0.5;
  var arr = [];
  var cnt = 0;
  ...
  function resize() {
    ...
    c.height = h = window.innerHeight*(2.5);
    c.style.position = 'absolute';
    c.style.left = (window.innerWidth - w) *
      .01 + 'px';
    c.style.top = (window.innerHeight*2.5 - h) *
      .01 + 'px';
  }
  ...
  function draw() {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      s.fillStyle = rndCol();
      s.beginPath();
      s.arc(splot.x, splot.y, splot.r, 0, Math.PI * 2, true);
      s.shadowBlur = 80;
      s.shadowOffsetX = 2;
      s.shadowOffsetY = 2;
      s.shadowColor = rndCol();
