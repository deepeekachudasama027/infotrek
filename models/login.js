...
pool.query(
   "CREATE TABLE IF NOT EXISTS login_clueminati (rollno int PRIMARY KEY ,cur_id int default 1 ,password varchar (50), score int default 1000,curhint int default 0, total_hit int default 0, wrong_hit int default 0,cur_img_time  bigint default 0,first_login bigint default 0 )",
  (err, result) =>{
    if (err) throw err;
  }
);
...
pool.query(
 ...
 pool.query(
  "CREATE TABLE IF NOT EXISTS question_clueminati (question varchar (150),id int PRIMARY KEY,answer varchar (150), hint1 varchar (300),hint2 varchar (300),hint3 varchar (300) , first_time bigint default 0, flag int default 0)",
 (err, result) =>{
   if (err) throw err;
 }
);
